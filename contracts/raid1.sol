...
contract Raid1 is Initializable, AccessControlUpgradeable {
    ...
    */
    ...
    uint256 public constant NUMBERPARAMETER_AUTO_BOSSPOWER_PERCENT =
        uint256(keccak256("BOSSPOWER_PERCENT"));
    ...
    event RewardedDustLB(
        uint256 indexed raidIndex,
        address indexed user,
        uint32 amount
    );
    event RewardedDust4B(
        uint256 indexed raidIndex,
        address indexed user,
        uint32 amount
    );
    event RewardedDust5B(
        uint256 indexed raidIndex,
        address indexed user,
        uint32 amount
    );
    event RewardedWeapon(
        uint256 indexed raidIndex,
        address indexed user,
        uint8 stars,
    ...
    function doRaidAuto() public restricted {
        ...
        uint256 power = ABDKMath64x64
            .divu(numberParameters[NUMBERPARAMETER_AUTO_BOSSPOWER_PERCENT], 100)
    ...
    function joinRaid(uint256 characterID, uint256 weaponID) public {
        ...
        uint256[] memory raiderIndices = raidParticipantIndices[raidIndex][
            msg.sender
        ];
        for (uint256 i = 0; i < raiderIndices.length; i++) {
            require(
                raidParticipants[raidIndex][raiderIndices[i]].wepID != weaponID,
                "This weapon is already used in the raid"
            );
            require(
                raidParticipants[raidIndex][raiderIndices[i]].charID !=
                    characterID,
        ...
        (
            uint8 charTrait,
            uint24 basePowerLevel, /*uint64 timestamp*/
        ...
        (
            ...
            uint24 weaponBonusPower, /*uint8 weaponTrait*/

        ) = weapons.getFightDataAndDrainDurability(
                weaponID,
                charTrait,
                uint8(durabilityCost),
                true
            );
        ...
        raidParticipantIndices[raidIndex][msg.sender].push(
            raidParticipants[raidIndex].length
        );
        raidParticipants[raidIndex].push(
            Raider(
                msg.sender,
                characterID,
                weaponID,
    ...
    function completeRaidWithSeed(uint256 seed) internal {
        ...
        uint256 roll = RandomUtil.randomSeededMinMax(
            0,
            raidPlayerPower[raidIndex] + bossPower,
            seed
        );
        ...
        if (outcome == STATUS_WON) {
            ...
            Raider memory trinketWinner = raidParticipants[raidIndex][
                seed % raidParticipants[raidIndex].length
            ];
            uint8 trinketStars = getTrinketStarsFromSeed(seed);
            uint256 trinketEffect = (seed / 100) % 5;
            uint256 tokenID = RaidTrinket(links[LINK_TRINKET]).mint(
                trinketWinner.owner,
                trinketStars,
                trinketEffect
            );
            emit RewardedTrinket(
                raidIndex,
                trinketWinner.owner,
                trinketStars,
                trinketEffect,
    ...
    function getTrinketStarsFromSeed(uint256 seed)
        private
        pure
        returns (uint8 stars)
    {
    ...
    function getPlayerFinalPower(
        ...
        if (isTraitEffectiveAgainst(charTrait, bossTrait))
            return
                uint24(
                    ABDKMath64x64.divu(1075, 1000).mulu(uint256(playerPower))
                );
        return playerPower;
    }
    ...
    function claimReward(uint256 claimRaidIndex) public {
        ...
        require(
            victory || raidStatus[claimRaidIndex] == STATUS_LOST,
            "Raid not over"
        );
        require(
            raidRewardClaimed[claimRaidIndex][msg.sender] == false,
            "Already claimed"
        );

        uint256[] memory raiderIndices = raidParticipantIndices[claimRaidIndex][
            msg.sender
        ];
        require(
            raiderIndices.length > 0,
            "None of your characters participated"
        );

        uint256 earlyBonusCutoff = raidParticipants[claimRaidIndex].length /
            2 +
            1; // first half of players
        // we grab raider info (power) and give out xp and raid stats
        for (uint256 i = 0; i < raiderIndices.length; i++) {
            uint256 raiderIndex = raiderIndices[i];
            Raider memory raider = raidParticipants[claimRaidIndex][
                raiderIndex
            ];
            ...
            if (victory) {
                distributeRewards(
                    ...
                    raiderIndex,
                    ABDKMath64x64.divu(
                        earlyMultiplier.mulu(raider.power),
                        raidPlayerPower[claimRaidIndex] /
                            raidParticipants[claimRaidIndex].length
                    )
                );
            }
            characters.processRaidParticipation(
                raider.charID,
                victory,
                uint16(earlyMultiplier.mulu(xpReward))
            );
        }
        ...
        if (bonusXpRoll <= 100) {
            ...
            if (bonusXpRoll > 50) {
                ...
                distributeBonusXp(
                    ...
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(25, 100).mulu(xpReward))
                );
            } else if (bonusXpRoll > 20) {
                // 1.5% for +150% xp
                distributeBonusXp(
                    msg.sender,
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(150, 100).mulu(xpReward))
                );
            } else if (bonusXpRoll > 10) {
                // 0.5% for +275% xp
                distributeBonusXp(
                    msg.sender,
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(275, 100).mulu(xpReward))
                );
            } else if (bonusXpRoll > 4) {
                // 0.3% for +525% xp
                distributeBonusXp(
                    msg.sender,
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(525, 100).mulu(xpReward))
                );
            } else if (bonusXpRoll > 1) {
                // 0.15% for +1150% xp
                distributeBonusXp(
                    msg.sender,
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(1150, 100).mulu(xpReward))
                );
            } else {
                // 0.05% for +2400% xp
                distributeBonusXp(
                    ...
                    claimRaidIndex,
                    raiderIndex,
                    uint16(ABDKMath64x64.divu(2400, 100).mulu(xpReward))
                );
            }
        }
    }

    function distributeBonusXp(
        address claimant,
        uint256 claimRaidIndex,
        uint256 raiderIndex,
        uint16 amount
    ...
    function distributeKeyBox(address claimant, uint256 claimRaidIndex)
        private
    {
        uint256 tokenID = KeyLootbox(links[LINK_KEYBOX]).mint(claimant);
        emit RewardedKeyBox(claimRaidIndex, claimant, tokenID);
    }

    function distributeJunk(
        address claimant,
        uint256 claimRaidIndex,
        uint8 stars
    ) private {
        uint256 tokenID = Junk(links[LINK_JUNK]).mint(claimant, stars);
        emit RewardedJunk(claimRaidIndex, claimant, stars, tokenID);
    }

    function distributeWeapon(
        address claimant,
        uint256 claimRaidIndex,
        uint256 seed,
        uint8 stars
    ) private {
        uint256 tokenID = weapons.mintWeaponWithStars(
            claimant,
            stars,
            seed / 100,
            100
    ...
    function setNumberParameter(uint256 paramIndex, uint256 value)
        public
        restricted
    {
    ...
    function getNumberParameter(uint256 paramIndex)
        public
        view
        returns (uint256)
    {
    ...
    function getRaidParticipantCount(uint256 index)
        public
        view
        returns (uint256)
    {
    ...
    function getEligibleRewardIndexes(uint256 startIndex, uint256 endIndex)
        public
        view
    ...
    function getParticipatingCharacters()
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory indices = raidParticipantIndices[raidIndex][
            msg.sender
        ];
    ...
    function getAccountsRaiderIndexes(uint256 index)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getAccountsPower(uint256 index) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < raiderIndexes.length; i++) {
            totalAccountPower += raidParticipants[index][raiderIndexes[i]]
                .power;
        }
        return totalAccountPower;
    }

    function canJoinRaid(uint256 characterID, uint256 weaponID)
        public
        view
        returns (bool)
    {
    ...
    function haveEnoughEnergy(uint256 characterID, uint256 weaponID)
        public
        view
        returns (bool)
    {
    ...
    function isCharacterRaiding(uint256 characterID)
        public
        view
        returns (bool)
    {
