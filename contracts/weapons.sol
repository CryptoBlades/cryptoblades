...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function mint(address minter, uint256 seed, uint256 choosenElement) public restricted returns(uint256) {
        uint256 stars;
        ...
        return mintWeaponWithStars(minter, stars, seed, choosenElement);
    }

    function mintWeaponWithStars(address minter, uint256 stars, uint256 seed, uint256 choosenElement) public restricted returns(uint256) {
        ...
        return performMintWeapon(minter,
            getRandomProperties(stars, seed, choosenElement),
            stat1,
            stat2,
            stat3,
    ...
    function getRandomProperties(uint256 stars, uint256 seed, uint256 choosenElement) public pure returns (uint16) {
        uint256 trait;
        if (choosenElement == 4) {
            trait = ((RandomUtil.randomSeededMinMax(0,3,RandomUtil.combineSeeds(seed,1)) & 0x3) << 3);
        } else {
            trait = ((choosenElement & 0x3) << 3);
        }
