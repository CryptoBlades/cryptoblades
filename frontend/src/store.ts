  ...
  return new Vuex.Store<IState>({
    ...
    actions: {
      ...
      async mintWeaponN({ state, dispatch }, {num, chosenElement, chosenElementFee}) {
        ...
        if(!hasPaidForMintBefore) {
          await CryptoBlades.methods
            .payForMint(Weapons.options.address, num, chosenElementFee)
            .send({
              from: state.defaultAccount,
            });
        }
      ...
      async mintWeapon({ state, dispatch }, {chosenElement, chosenElementFee}) {
        ...
        if(!hasPaidForMintBefore) {
          await CryptoBlades.methods
            .payForMint(Weapons.options.address, 1, chosenElementFee)
            .send({
              from: state.defaultAccount,
            });
        }
