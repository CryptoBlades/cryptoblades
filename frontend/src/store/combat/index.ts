...
const defaultCallOptions = (rootState: IState) => ({
...
const combat = {
  ...
  mutations: {
    ...
    updateTargets(
      state: ICombatState,
      { characterId, targets }: { characterId: number; targets: ITarget[] }
    ) {
  ...
  actions: {
    ...
    async fetchTargets(
      ...
      const targets = await rootState
        .contracts()
        .CryptoBlades!.methods.getTargets(characterId)
        .call(defaultCallOptions(rootState));
    ...
    async getCharacterPower(
      ...
      return await Characters.methods
        .getPower(characterId)
        .call({ from: rootState.defaultAccount, gasPrice: getGasPrice() });
    },

    async fetchExpectedPayoutForMonsterPower(
      { rootState }: { rootState: IState },
      { power }: { power: string | number }
    ) {
      const { CryptoBlades } = rootState.contracts();
      if (!CryptoBlades) return;
      return await CryptoBlades.methods
        .getTokenGainForFight(power)
        .call(defaultCallOptions(rootState));
    },
    ...
    async fetchFightRewardXp({
      ...
      const xps = await CryptoBlades.methods
        .getXpRewards(rootState.ownedCharacterIds.map((x) => x.toString()))
        .call(defaultCallOptions(rootState));
      ...
      commit(
        "updateXpRewards",
        { xpRewards: _.fromPairs(xpCharaIdPairs) },
        { root: true }
      );
      return xpCharaIdPairs;
    },

    async fetchCharacterStamina(
      { rootState, commit }: { rootState: IState; commit: Commit },
      characterId: number
    ) {
      const staminaString = await rootState
        .contracts()
